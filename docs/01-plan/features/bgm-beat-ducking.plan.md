...
| 5d-core | 덕킹 엔벨로프 순수 코어 `computeDuckingEnvelope`(발화 구간→base↔duck 키프레임, attack/release 램프, 병합) + 테스트 8개 | 소~중 | **완료** |
| 5d-apply | 볼륨 키프레임 적용(premiere.ts 오디오 Volume>Level ComponentParam 쓰기) + 자막 발화 구간 연결 + UI. Host 키프레임 쓰기 가용성 미탐침 | 중~대 | 대기 |
